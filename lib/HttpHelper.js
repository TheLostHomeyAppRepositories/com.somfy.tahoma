...
module.exports = class HttpHelper extends SimpleClass
{
    ...
    async checkAuthentication(doLogin)
    {
        ...
        {
            const oAuthTimeLeft = this.tahoma.getOAuthTimeLeft();
            if (doLogin && (oAuthTimeLeft > -1) && (oAuthTimeLeft < 60))
            {
                this.authenticated = false;
                await this.tahoma.refreshOAuth();
            }
            ...
            if (error.response)
            {
                if (error.response.status === 401)
                {
                    if (firstTime)
                    {
                        ...
                        if (this.localLogin)
                        {
                            this.homey.app.localBearer = null;
                        }
                        if (this.homey.app.infoLogEnabled)
                        {
                            this.homey.app.logInformation('post',
    ...
    async reAuthenticate()
    {
        ...
        try
        {
            ...
            {
                await this.logout();
                await this.tahoma.getLocalAuthCode(username, password, this.tahoma.devicePin, this.tahoma.port, null, this.tahoma.homeyID);
            }
            else
            {
                await this.tahoma.login(username, password, 'default', loginMethod, false, true);
            }
