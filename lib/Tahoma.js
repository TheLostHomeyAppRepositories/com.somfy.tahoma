		...
		if (this.authenticated)
		{
			...
			if (this.homey.settings.get('debugMode'))
			{
				...
				if (simData)
				{
					...
					{
						// Return the simulated data as {devices: {local: []}}
						return { devices: { local: simData.devices.local } };
					}

					if (!this.localLogin && simData.devices && simData.devices.cloud)
					{
						// Return the simulated data as {devices: {cloud: []}}
						return { devices: { cloud: simData.devices.cloud } };
					}
