			...
			if (!bearer)
			{
				...
				if (newToken == null)
				{
					...
					const deletePromises = localTokenList
						...
						.map((token) => this.httpHelper.delete(`/config/${devicePin}/local/tokens/${token.uuid}`, {}, false));
		...
		if (loginAttempts > 0)
		{
			if (this.homey.app.infoLogEnabled)
			{
				this.homey.app.logInformation(`Tahoma Cloud Login for ${type} connection`,
				{
					...
					stack: `Login attempts: ${loginAttempts}, next Login time: ${lastTime.toJSON()}, current time: ${dateNow.toJSON()}`,
				});
			}

			if (loginTime > dateNow)
			{
				this.lastLoginIssue = loginAttempts <= maxAttempts ? `Please leave 1 minutes between cloud login attempts, next Login time: ${lastTime.toJSON()}` : `Far Too many ${type} login attempts (blocked for ${fromPro ? '15 minutes' : '24 hours'})`;
