...
module.exports = class Tahoma extends Homey.SimpleClass
{
    ...
    async getLocalAuthCode(username, password, devicePin, port, bearer, homeyID)
    {
        try
        {
            if (!bearer)
            {
                ...
                {
                    // Already logged in therefore we need to log out
                    await this.logout();
                }
                ...
                this.localTokenList = await this.httpHelper.get(`/config/${devicePin}/local/tokens/devmode`, {}, false);
            ...
            this.httpHelper.authenticated = true;
    ...
    {
        if (await this.httpHelper.checkAuthentication(false) && !this.httpHelper.localLogin)
        {
    ...
    async deleteLocalToken(devicePin, uuid)
    {
        if (await this.httpHelper.checkAuthentication(false) && !this.httpHelper.localLogin)
        {
            return await this.httpHelper.delete(`/config/${devicePin}/local/tokens/${uuid}`, {}, false);
        }
    ...
    async logout()
    {
        ...
        try
        {
            if (!this.httpHelper.localLogin && await this.httpHelper.checkAuthentication(false))
            {
    ...
    async getDeviceStates(deviceUrl)
    {
        if (await this.httpHelper.checkAuthentication(false))
        {
            if (this.httpHelper.localLogin)
            {
                const states = await this.httpHelper.get(`/setup/devices/${encodeURIComponent(deviceUrl)}`);
