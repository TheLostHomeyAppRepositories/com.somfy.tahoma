...
module.exports = class Tahoma extends Homey.SimpleClass
{
	...
	async getSetupSnapshot()
	{
		...
		const app = this.getAppInstance();
		const setupOID = app && app.somfySetupOID ? String(app.somfySetupOID) : '';
		...
		try
		{
			const accountSetups = await this.getWithReAuth('/enduser/setups');
		...
		snapshot.gateways = mergeUniqueByKey([].concat(embeddedGateways, fetchedGateways), (gateway) =>
		{
		...
		snapshot.devices = mergeUniqueByKey([].concat(embeddedDevices, fetchedDevices), (device) =>
		{
			if (device.deviceURL)
			{
