...
module.exports = class Tahoma extends Homey.SimpleClass
{
	...
	async login(username, password, region, oauthLogin, fromPro, fromReAuthenticate = false, setupOID = '')
	{
		...
		const authSource = `Tahoma Cloud Login #${loginStartNumber} for ${type} connection${authSession ? ` [${authSession}]` : ''}`;
	...
	async getSetupSnapshot()
	{
		...
		{
			const primaryDeviceGateways = new Set(this.annotateDevicesWithGateway([].concat(embeddedDevices, fetchedDevices))
				.map((device) => (device && device.gatewayId ? String(device.gatewayId) : ''))
			...
			for (const setup of accountSetups)
			{
				...
				const attempt = { setupOID: candidateOID, fetched: false, deviceCount: 0, gatewayCount: 0 };
				try
				{
					await scopedClient.login(this.username, this.password, this.region, this.oauthLogin, true, true, candidateOID);
					const scopedSetup = await scopedClient.getWithReAuth('/setup');
					if (scopedSetup && Array.isArray(scopedSetup.devices))
					{
						const scopedDevices = this.annotateDevicesWithGateway(scopedSetup.devices);
						const introducesNewGateway = scopedDevices.some((device) =>
						{
							const gatewayId = device && device.gatewayId ? String(device.gatewayId) : '';
							return gatewayId && !primaryDeviceGateways.has(gatewayId);
						});

						if (introducesNewGateway || (primaryDeviceGateways.size <= 1))
						{
							crossSetupDeviceItems.push(...scopedSetup.devices);
							if (Array.isArray(scopedSetup.gateways))
							{
								crossSetupGatewayItems.push(...scopedSetup.gateways);
							}

							attempt.fetched = true;
							attempt.deviceCount = scopedSetup.devices.length;
							attempt.gatewayCount = Array.isArray(scopedSetup.gateways) ? scopedSetup.gateways.length : 0;
						}
					}
				}
				catch (error)
		...
		this.lastSetupDiscovery = {
			...
			crossSetupSuccessCount: crossSetupAttempts.filter((attempt) => attempt && attempt.fetched).length,
