...
//            await this.logout();
            ...
            if (this.homey.app.infoLogEnabled)
            {
                ...
                this.homey.app.logInformation('Tahoma Local Login',
                {
                    message: `Ready for local connections to https://${hostName}`,
                });
            }
    ...
    async login(username, password, region, oauthLogin, fromPro, fromReAuthenticate = false)
    {
        ...
        if (this.homey.app.infoLogEnabled)
        {
            ...
            {
                message: 'Starting Cloud Login',
                stack: `Login attempts: ${loginAttempts} last Login time: ${Date(loginTime).toString()}`,
            });
        }
