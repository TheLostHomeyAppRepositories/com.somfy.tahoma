...
module.exports = class Tahoma extends Homey.SimpleClass
{
    ...
    async getLocalAuthCode(username, password, region, devicePin, port, bearer, homeyID)
    {
        try
        {
            ...
            if (this.homey.app.infoLogEnabled)
            {
                ...
                this.homey.app.logInformation('Tahoma Login',
                {
                    message: `Start local login to https://${hostName}`,
                });
            }
    ...
    async login(username, password, region, oauthLogin, fromPro, fromReAuthenticate = false)
    {
        ...
        {
            throw (loginAttempts <= maxAttempts ? new Error('Please leave 1 minutes between login attempts') : new Error(`Far Too many ${type} login attempts (blocked for ${fromPro ? '15 minutes' : '24 hours'})`));
        }
        ...
        try
        {
            ...
            if (!oauthLogin)
            {
                ...
                this.homey.settings.unset('loginAttemptsOA');
                if (this.homey.app.infoLogEnabled)
                {
            ...
            else
            {
                ...
                this.homey.settings.unset('loginAttempts');
                this.username = username;
                this.password = password;
                this.region = region;
                this.oauthLogin = oauthLogin;
        ...
        {
            this.homey.app.logInformation(`Tahoma ${type} Login`, `Error: ${error.message}`);
            if (error.response && error.response.status === 429)
            {
                // Too many request. Header should contain Retry-After: time in seconds
                if (error.response.headers['retry-after'])
    ...
    {
        // Don't try reauth
        await this.httpHelper.get('/currentUserId', this.httpsAgent ? { httpsAgent: this.httpsAgent } : {});
    }
