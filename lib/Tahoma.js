...
module.exports = class Tahoma extends Homey.SimpleClass
{
    ...
    constructor(Homey, Local)
    {
        ...
        if (Local)
        {
            this.local_httpsAgent = new https.Agent({ ca: fs.readFileSync('/assets/overkiz-root-ca-2048.crt') });
        }
    ...
    async postWithReAuth(uri, config, data)
    {
        ...
        catch (error)
        {
            if (error.response && error.response.status === 401)
            {
                ...
                if (this.localLogin)
                {
                    const bearer = await this.getLocalAuthCode(this.username, this.password, this.devicePin, this.port, null, this.homeyID);
                    this.homey.settings.set('localBearer', bearer);
                    this.homey.app.localBearer = bearer;
                }
                else
                {
                    await this.login(this.username, this.password, this.linkurl, this.oauthLogin, true, true);
                }
    ...
    async getWithReAuth(uri)
    {
        ...
        catch (error)
        {
            if (error.response && error.response.status === 401)
            {
                ...
                if (this.localLogin)
                {
                    const bearer = await this.getLocalAuthCode(this.username, this.password, this.devicePin, this.port, this.homeyID);
                    this.homey.settings.set('localBearer', bearer);
                    this.homey.app.localBearer = bearer;
                }
                else
    ...
    async getLocalAuthCode(username, password, devicePin, port, bearer, homeyID)
    {
        try
        {
            ...
            if (!bearer)
            {
                ...
                await this.login(username, password, 'local_auth', true);
            ...
            this.local_httpsAgent = new https.Agent({ ca: fs.readFileSync('/assets/overkiz-root-ca-2048.crt'), servername:  `${devicePin}.local`});
    ...
    async getLocalTokens(devicePin)
    {
        ...
        return await this.httpHelper.get(`/config/${devicePin}/local/tokens/devmode`);
    }
    ...
    async deleteLocalToken(devicePin, uuid)
    {
        ...
        return await this.deleteWithReAuth(`/config/${devicePin}/local/tokens/${uuid}`);
    }

    // Throws an error if the login fails
    async login(username, password, linkurl, oauthLogin, fromPro, fromReAuthenticate = false)
    {
        ...
        if (this.loginTimer15mID === null)
        {
            ...
            this.loginTimer15mID = this.homey.setTimeout(() =>
            {
                ...
                this.loginTimer15mID = null;
            }, fromPro ? 900000 : 43200000);
        }

        // Allow up to 2 login attempts in 20 seconds or 6 in 5 minutes
        if ((this.loginAttempts60s > 2) || (this.loginAttempts15m > 6))
        {
            throw (this.loginAttempts15m > 6 ? new Error('Far Too many login attempts (blocked for 15 minutes)') : new Error(`Too many login attempts (blocked for ${fromPro ? '60 seconds': '12 hours'})`));
        }
        ...
        {
            this.homey.app.logInformation('Tahoma Login', `Error: ${error.message}`);
            throw (error);
        }
    }
    ...
    {
        const apiVersion = await this.httpHelper.get('apiVersion', { httpsAgent: this.httpsAgent }, false);
        this.homey.app.logInformation('Tahoma Login',
        {
    ...
    async getUserId()
    {
        return this.getWithReAuth('/currentUserId');
    }
